...
shopt -u nocasematch
# Document lengths to sweep (tokens)
readonly DOC_LENGTHS=(2176 4224 8320 16512 33024 65792 98432 129152)
...
readonly KV_TRANSFER_CONFIG='{"kv_connector":"LMCacheConnectorV1","kv_role":"kv_both"}'
readonly LMCACHE_CONFIG="/root/lmcache-s3-nixl.yaml"
...
main() {
    ...
    cat > "${SUMMARY_FILE}" <<EOF
vLLM + LMCache S3 Benchmark (Doc Length Sweep)
Timestamp: ${TIMESTAMP}
Model: ${MODEL}
TP size: ${TP_SIZE}
...
EOF
    ...
    for doc_length in "${DOC_LENGTHS[@]}"; do
        ...
        log_info "=== Document length ${doc_length}: warming S3 cache ==="
        clear_s3_cache
        run_benchmark_once \
            "${doc_length}" \
            1 \
            "${doc_dir}" \
            "doc${doc_length}_warm" \
            "${KV_TRANSFER_CONFIG}" \
            "${LMCACHE_CONFIG}" \
            "true" \
            "true" \
            "doc${doc_length}_warm_session"

        sleep 5

        log_info "=== Document length ${doc_length}: reusing warmed S3 cache ==="
        run_benchmark_once \
            "${doc_length}" \
            2 \
            "${doc_dir}" \
            "doc${doc_length}_cached" \
            "${KV_TRANSFER_CONFIG}" \
            "${LMCACHE_CONFIG}" \
            "true" \
            "true" \
            "doc${doc_length}_cached_session"
